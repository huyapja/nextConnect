...
import {
  ImageChannelDelete,
  imageChannelDeleteAtom
} from '@/components/feature/channel-details/change-image-channel/ImageChannelDelete'
...
export const ChannelIcon = ({
  ...
  const effectiveGroupImage = groupImage?.trim() || undefined

  useEffect(() => {
    if (!open) {
      setUploadState({ open: false, groupID: undefined })
      setDeleteState({ open: false, groupID: undefined })
    }
  }, [open])

  useEffect(() => {
    if (!effectiveGroupImage && open) {
      setOpen(false)
    }
  }, [effectiveGroupImage, open])

  const renderImage = (clickable = false) => (
    <img
      src={effectiveGroupImage}
      alt='Channel'
    ...
    if (canUserView) {
      return (
        <>
          ...
          <ImageChannelDelete setModalOpen={setOpen} />
        </>
      )
    }
