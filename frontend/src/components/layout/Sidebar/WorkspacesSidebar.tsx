...
const WorkspaceItem = ({ workspace }: { workspace: WorkspaceFields & { unread_count: number } }) => {
  ...
  return (
    <HStack position='relative' align='center' className='group'>
      <Box
        className={clsx(
          'w-1.5 bg-gray-12 rounded-r-full dark:bg-gray-12 absolute sm:-left-3 -left-3.5 group-hover:h-4 transition-all duration-200 ease-ease-out-cubic',
