...
export const ImageMessageBlock = memo(
  ({ message, isScrolling = false, user, onRetry, onRemove }: ImageMessageProps) => {
    ...
    const fileName = useMemo(() => {
      ...
      if (message.fileMeta?.name) {
        return message.fileMeta.name
      }
    ...
    const showRetryButton = isError || (isPending && !!imageSrc)
    ...
    return (
      <Flex direction='column' gap='1'>
        ...
        >
          ...
          {showRetryButton && (
            <Box
              className='absolute bottom-2 right-2 flex gap-1 items-center z-[999]'
              style={{
          ...
          {isPending && !showRetryButton && (
            ...
            >
              <span style={{ color: '#ccc', fontSize: '12px' }}>Đang gửi...</span>
            </Box>
          )}
        </Box>
