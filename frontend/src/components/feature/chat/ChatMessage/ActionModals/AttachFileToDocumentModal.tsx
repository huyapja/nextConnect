    ...
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Flex gap='2' direction='column' width='100%'>
          ...
          <Callout.Root>
            <Callout.Icon>{<FileExtensionIcon ext={getFileExtension((message as FileMessage).file)} />}</Callout.Icon>
            <Callout.Text>
              <Link href={(message as FileMessage).file}>{getFileName((message as FileMessage).file)}</Link>
            </Callout.Text>
          </Callout.Root>
          <Box width='100%'>
