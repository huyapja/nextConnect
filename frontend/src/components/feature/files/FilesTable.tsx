...
export const FilesTable = ({ data }: { data: FileInChannel[] }) => {
  ...
  if (isDesktop) {
    ...
    return (
      <Flex direction='column' gap='3' className='overflow-scroll pb-4'>
        {data?.map((file: FileInChannel) => (
          <Flex
            key={file.name}
            align='center'
            justify='between'
            className='border-b border-gray-4 dark:border-gray-6`} pb-3'
          >
      ...
      <HoverCard.Content>
        <Inset side='all' mb='0' className='-mb-6'>
          <img
            ...
            alt={file.file_name}
            height={file.thumbnail_height || '200px'}
            width={file.thumbnail_width || '200px'}
