...
const ViewFilesContent = () => {
  ...
  const { data: count, error: countError } = useFrappeGetCall<{ message: number }>(
  ...
  const { data, error, isLoading } = useFrappeGetCall<{ message: FileInChannel[] }>(
    'raven.api.raven_message.get_all_files_shared_in_channel',
    {
      channel_id: channelID,
      file_name: debouncedText,
      file_type: fileType === 'any' ? undefined : fileType,
        ...
        <Flex justify='between' gap='2' className='flex-col sm:flex-row'>
          <Flex gap='2' className='flex-col sm:flex-row sm:items-center'>
            ...
            >
              ...
              <TextField.Slot side='right'>{isLoading && <Loader />}</TextField.Slot>
            </TextField.Root>
