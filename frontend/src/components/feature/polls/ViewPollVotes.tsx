    ...
    return (
      <Dialog.Root open={open} onOpenChange={setOpen}>
        ...
        <Dialog.Content className={clsx(DIALOG_CONTENT_CLASS, 'max-h-[80vh]')}>
          <ViewPollVotesModalContent onClose={onClose} poll={poll} />
        </Dialog.Content>
      </Dialog.Root>
    )
  } else {
...
const ViewPollVotesModalContent = ({ onClose, poll }: ViewPollVotesModalContentProps) => {
  ...
  const { data, error } = useFrappeGetCall<{ message: PollVotesResponse }>(
    'raven.api.raven_poll.get_all_votes',
    {
  ...
  return (
    <Flex gap={'2'}>
      ...
      <div className='flex flex-col text-ellipsis justify-center border-b border-gray-6 w-full pb-2 group-last:pb-0 group-last:border-b-0'>
        <Text as='span' className='block' weight='medium' size='2'>
          {user?.full_name}
        </Text>
      </div>
    </Flex>
