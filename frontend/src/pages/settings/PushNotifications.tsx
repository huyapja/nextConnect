        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <SettingsContentContainer>
            ...
            <Box className='max-w-96'>
              <Label isRequired htmlFor='push_notification_service'>
                {__('Push Notification Service')}
              </Label>
              <Controller
                control={control}
                ...
                render={({ field }) => (
                  ...
                  >
                    ...
                    <Select.Content>
                      <Select.Item value='Raven'>{__('Raven Cloud')}</Select.Item>
                      <Select.Item value='Frappe Cloud'>{__('Frappe Cloud')}</Select.Item>
                    </Select.Content>
                  </Select.Root>
                )}
              />
            ...
            {isRavenCloud ? (
              <Stack gap='3'>
                <Text size='2'>
                  To get started with Raven Cloud, you need to first{' '}
                  <Link href='https://cloud.ravenchat.ai' target='_blank'>
                    create an account <FiExternalLink />
                  </Link>{' '}
                  and get your API Key and API Secret.
                </Text>
                <Box>
                  <Label htmlFor='push_notification_server_url' isRequired>
                  ...
                  />
                  {errors?.push_notification_server_url && (
                    <ErrorText>{errors.push_notification_server_url?.message}</ErrorText>
                  )}
                  <HelperText size='2'>
                ...
                <Box>
                  ...
                  />
                  {errors?.push_notification_api_key && (
                    <ErrorText>{errors.push_notification_api_key?.message}</ErrorText>
                  )}
                </Box>
                <Box>
                  <Label htmlFor='push_notification_api_secret' isRequired>
                    API Secret
                  </Label>
                  <TextField.Root
                    className={'w-48 sm:w-96'}
                  ...
                  />
                  {errors?.push_notification_api_secret && (
                    <ErrorText>{errors.push_notification_api_secret?.message}</ErrorText>
                  )}
                </Box>
              </Stack>
            ) : null}
            ...
            <div className='flex gap-2'>
              {isRavenCloud &&
                ravenSettings?.push_notification_service === 'Raven' &&
                ravenSettings?.push_notification_server_url && (
              ...
              {isRavenCloud &&
                ravenSettings?.push_notification_service === 'Raven' &&
                ravenSettings?.push_notification_server_url &&
                ravenSettings?.vapid_public_key && (
                  <Button
                    onClick={() =>
                      call.post('raven.api.notification.sync_user_tokens_to_raven_cloud').then(() => {
              ...
              {!isRavenCloud && (
                <Button asChild color='gray' variant='outline' className='not-cal'>
                  <Link color='gray' target='_blank' underline='none' href={`/app/push-notification-settings`}>
...
const RegisterSiteButton = ({ mutate, ravenSettings }: { mutate: VoidFunction; ravenSettings: RavenSettings }) => {
  ...
  return (
    <Button onClick={registerSite} disabled={loading} variant='soft' type='button' className='not-cal'>
      {ravenSettings.vapid_public_key ? 'Re-Register Site on Raven Cloud' : 'Register Site on Raven Cloud'}
    </Button>
  )
}
